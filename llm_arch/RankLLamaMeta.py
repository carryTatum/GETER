...
class RankLLamaMeta:
    def __init__(self, config):
        super(RankLLamaMeta, self).__init__(config)
...
class RankMetaForCausalLLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if labels is not None:
                cur_new_labels.append(torch.full((cur_graph_embeds.shape[0],), -100, dtype=labels.dtype, device=labels.device))
        ...
        if attention_mask is not None:
            new_attention_mask_pad_left = torch.full((attention_mask.shape[0], new_input_embeds.shape[1] - input_ids.shape[1]),
